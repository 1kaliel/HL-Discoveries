# License and third-party notice

Copyright 2026 HL-Discoveries contributors.

Except where otherwise identified, original documentation and copyrightable database
contributions in this repository are licensed under the **Creative Commons Attribution
4.0 International License (CC BY 4.0)**.

- [License summary](https://creativecommons.org/licenses/by/4.0/)
- [Full legal terms](https://creativecommons.org/licenses/by/4.0/legalcode.en)

Suggested attribution: "HL-Discoveries contributors — https://github.com/1kaliel/HL-Discoveries
(CC BY 4.0)." Identify modifications when redistributing an adapted version.

This license applies only to rights contributors can grant. It does not license Hogwarts
Legacy game assets, executable code, trademarks, or other third-party intellectual property.
Asset identifiers describe referenced material; the referenced asset files are not distributed
here. Nothing here creates restrictions on facts that are not subject to copyright.

Executable code and its accompanying documentation under
`tools/ck-thumbnail-extractor` are separately available under the MIT License in that
directory. This exception does not license any game asset a user processes with the tool.

No warranty of accuracy, compatibility, completeness or fitness for any purpose is provided.
